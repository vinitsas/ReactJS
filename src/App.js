...
class App extends Component {
	...
	fileUpload = (event) => {
		...
		for(var i=0;i<uploaded_images.length;i++){
			...
			if(ext==="jpg" || ext==="jpeg" || ext==="png" || ext==="gif"){
				...
				axios.post('http://localhost:9000/php-upload/upload.php', fd)
				.then(res=>
				{
					// console.log(res);
				}
	...
	fetchUsers() {
	  ...
	  fetch('http://localhost:9000/php-upload/get-images')
	...
	removeImage = (id) => {
		...
		axios.post('http://localhost:9000/php-upload/remove?id='+id)
